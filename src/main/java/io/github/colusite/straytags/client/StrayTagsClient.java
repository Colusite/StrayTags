...
public class StrayTagsClient implements ClientModInitializer {
    ...
    public static Component rebuildWithPrefixSuffix(String rawString, String cleanedString, Component modified) {
        ...
        for (int rawIdx = 0; rawIdx < rawString.length() && cleanIdx < cleanedString.length(); rawIdx++) {
            ...
            String charStr = String.valueOf(rawChar);
            if (charStr.replaceAll("[^a-zA-Z0-9_ \\[\\]]", "").isEmpty()) {
                continue;
            }
            ...
            if (cleanIdx == 0) {
                ...
                if (cleanIdx >= cleanedString.length()) break;
            }

            if (rawChar == cleanedString.charAt(cleanIdx)) {
                if (cleanStart == -1) {
                    cleanStart = rawIdx;
                }
                cleanEnd = rawIdx + 1;
        ...
        String rawPrefix = rawString.substring(0, cleanStart);
        String rawSuffix = rawString.substring(cleanEnd);
        ...
        if (verboseMode) {
            ...
            try {
                ...
                if (!matcher.matches()) {
                    ...
                    client.player.displayClientMessage(
                            Component.literal("§e[ST]   §7rank=§f" + (rank != null ? rank : "(none)")
